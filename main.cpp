...
//------------------------------------------------------
int A_STAR[32*32][2];// TABLEAU DU CHEMIN QU ON CHERCHE
int A_COUNT;// LE NOMBRE DE CASES DU CHEMIN
...
void update_(int start[2],int goal[2],int* indice,node* parent){
...
void update_voisin(node* current){
int i;
int** C;
C=carre(current->indice);
for(i=0;i<4;i++){
    if(into(C[i],Now,Now) && notinclosed(C[i]) ){
        update_(start,goal,C[i],current);
    }
}
}
...
return(in(int(event.button.x),int(event.button.y),rect.x,rect.x+rect.w,rect.y,rect.y+rect.h));
			...
			// retourche si le clic est dans une case

//------------------------------------------------------------------

//-------------------------------------------------------------------------------------
void SDIDI(SDL_Window * pWindow){
    int i,j;
    ...
    if ( pRenderer )
    {
        ...
        if ( Sprite1 )
            ...
            SDL_Texture* Texture8= SDL_CreateTextureFromSurface(pRenderer,Sprite8);    // Préparation du sprite
            if ( Texture1 )
            {
                if (w==1){
                    A_STAR_SOLUTION();
            ...
            SDL_DestroyTexture(Texture8);  // Libération de la mémoire associée à la texture
        }}
        else
        {
        ...
        SDL_FreeSurface(Sprite8);// Libération de la ressource occupée par le sprite
    }
    else
    {
